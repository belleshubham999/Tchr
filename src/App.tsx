...
import { analyzeNote, tutorChat } from './services/gemini';
...
export default function App() {
  ...
  const handleToolCall = (calls: any[]) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const aiResponse = await tutorChat(newChatMessages, messageToSend, handleToolCall, studyPlans);
  ...
  const handleAddNote = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const analysis = await analyzeNote(newNote.content);
      await db.createNote(
        user.id,
        newNote.title,
        newNote.content,
